...
Unfortunately there is already some duplication in the definitions of the child groups \texttt{lhs} and \texttt{rhs}: Both should always allow the exact same types. And on top of that it is not possible to nest expressions, because \texttt{lang.expBin} is not an allowed child for either of the sides. In a typical grammar we would simply introduce a new node of type like \texttt{lang.expAny} which would consist of a single child group that lists all permitted values (see the following listing~\ref{lst:grammar-expr-expBin-choiceNode}):
...
A tree for such a grammar would however contain nodes of type \texttt{expAny} that do not have any kind of possible representation for the end user: \pdfmarkupcomment[markup=Squiggly]{As there is no possible textual representation for such a node, it would appear as a completely empty block that can only host another block.}{Visual argument}

Therefore any kind of recurring choice between multiple node types is treated differently in these grammars: By introducing a pure virtual node type that is referrable by name in the grammar, but may never appear in an actual syntax tree. The textual grammar representation uses the keyword \texttt{typedef} to introduce such a type, the following listing~\ref{lst:grammar-expr-expBin-typedef} changes the definition of \texttt{nodeAny} to be such a virtual node. The definition of \texttt{expBin} can be unaffected by this change, altough the trees that are validated by the grammars are different.
...
\end{minted}
\caption{Final Grammar for expression \texttt{expBin}}
