...
The example programming languages that have been shown in this paper demonstrate, that the general approach of generating a block based development environment from a formal specification is versatile and fast. Once a runtime for a conventional programming language has been developed, it can be targeted by multiple block language approaches that may put emphasis on different aspects. Therefore the changing educational requirements leading to different relevant subsets of languages can be met by creating a new grammar with the builtin grammar editor.

So in its current state the environment we developed is self-hosting and allows the definition of alternate block languages for every runtime that is already implemented. Currently runtimes are available for \texttt{SQLite}, \texttt{JavaScript}, Regular Expressions, \texttt{GraphViz}, basic math equations based on \texttt{Nerdamer} and a quite complex (but ultimately artificial) \texttt{JavaScript}-variation of the Kara-approach called \texttt{Trucklino}\cite{popp_konzeption_2019}, which is also shown in figure~\ref{fig:js-runtime-trucklino}.
\printbibliography
