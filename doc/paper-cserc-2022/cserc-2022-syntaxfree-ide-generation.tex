...
\todo{Hier geht das viel zu schnell ab und du tauchst in deine Welt ein. Habe ersucht das etwas abzufedern mitd em ersten Satz, aber ist  noch nicht rund. Ich dachte auch, dass ein Beispiel für eine Grammatik kommt, aber das ist ja eher ein Beispiel für ein Wort. Das passt dadurch nicht richtig zu dem vorherigen Kapitel. Der Start mit the tree... is unklar. Sonst Kapitel vielleicht besser "A Binary Expression" nennen.

Es könnte vielleicht auch sinnvoll sein, davor noch einen einleitenden Abschnitt zu machen, in dem du sagst, was jetzt in den nächsten Kapiteln kommen wird. Du willst scheinbar deine GRammatik modellieren. Wäre vermutlich einfacher, wenn du sie einfach erstmal vorstellen würdest und danach dann die Features begründen würdest.}
As a first example language we model simple arithmetric expression, containing binary operators, like the comparison \texttt{numRattles == null}.

The tree in Listing~\ref{lst:ast-expr-binary} is shown in two equivalent representations: \texttt{JSON} and a rendered tree. It is a possible representation of the expression \texttt{numRattles == null} for any language that allows to\todo{Hier geht das viel zu schnell ab und du tauchst in deine Welt ein. Habe ersucht das etwas abzufedern mitd em ersten Satz, aber ist  noch nicht rund. Ich dachte auch, dass ein Beispiel für eine Grammatik kommt, aber das ist ja eher ein Beispiel für ein Wort. Das passt dadurch nicht richtig zu dem vorherigen Kapitel.}
As a first example language we model simple arithmetric expression, containing binary operators, like the comparison \texttt{numRattles == null}. compare a \texttt{null} value against a variable. It consists of three nodes: The root node is of type \texttt{lang.expBin}, the dot (\enquote{\texttt{.}}) is used as a separator between the \texttt{language} and the \texttt{name} of a node. The childgroup with the name \texttt{lhs} (\enquote{left hand side}) contains a node of type \texttt{lang.expVar} which is meant to refer to an existing variable on the left hand side of the expression. The property \texttt{name} is set to \texttt{numRattles}.
...
\todo{Ich wäre hier vorsichtig mit dem Begriff semantic (Bildunterschrift). Da ist ja nix semantisches drin.}
\begin{listing}[H]
\begin{minted}[]{typescript}
grammar "lang" {
  node "null" {
...
\todo{runtimes finde ich einen sehr merkwürdigen Begriff. Nutzt man eher für Laufzeiten. Ich würde Applications, Examples or Instances als Begriff vorschlagen.}
