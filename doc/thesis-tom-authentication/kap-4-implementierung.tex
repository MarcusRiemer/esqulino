...
In dieser Sektion wird sich ausgiebig\todo{Warum ist die Ausgiebigkeit wichtig?} mit der Implementierung der zuvor festgelegten Anforderungen auseinandergesetzt.
...
Die Funktionalitäten die Omniauth bietet sind jedoch für die Zielverwirklichung ausreichend, we{\ss}halb\todo{Typo} sich gegen Devise entschieden wurde. Aus diesem Grund mussten Funktionen wie zum Beispiel das Senden einer Bestätigungsmail selbst implementiert werden.
...
\todo[inline]{Listing läuft rechts raus}
...
Die nachfolgenden Beschreibungen der Attribute beziehen sich auf die identities\todo{Codebezeichner} Tabelle, welche in Abbildung \ref{fig:server-users-identities} dargestellt wird.
\begin{description}
	\item[uid]\hfill\\
...
\todo[inline]{Warum 2x ``Client'' und 2x ``BlattWerkzeug''?}
...
\begin{description}
	...
	Da es vorkommen kann, dass eine Verifizierungsmail nicht an der angegebene E-Mail ankommt, wurde hierfür eine Funktion im IdentitiesController\todo{Codebezeichner} geschrieben. Diese Funktion prüft die übermittelte E-Mail auf ihre Existens in der Blattwerkzeug-Datenbank und ob diese bereits verifiziert wurde. Sollte diese E-Mail nicht verifiziert sein, wird eine neue Verifizierungsmail versendet. Gleichzeitig wird jedoch auch eine Wartezeit von zwei Minuten im own\_data\todo{Codebezeichner} Attribut festgelegt. Die Wartezeit zum Versenden einer neuen Verifizierungsmail soll verhindern, dass eine E-Mail Adresse mit Verifizierungsmails überhäuft werden kann.
...
\subsubsection{may\_perform\todo{Lieber abstrakte Bezeichnung als Funktionsnamen}}
...
\todo[inline]{Screenshots zeigen zuviel irrelevanten Hintergrund, passen die Beschreibungen?}
...
\begin{description}
	...
	\begin{figure}
        ...
        \todo[inline]{Was sind die Kästen mit ``update\_all'' bzw. ``create'', ``update'', ``delete''?}
