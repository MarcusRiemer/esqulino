...
Wurzel der entwickelten Minisprache ist immer ein Programm, welches beliebig viele Prozedurdefinitionen (\inlinec{procedureDeclaration}), sowie mehrere Anweisungen (\inlinec{statement}) eines Hauptprogramms als Kindknoten aufnehmen kann. Prozeduren lassen sich nur in diesem Deklarationsteil definieren. Vom alternativen Vorgehen, Prozedurdefinitionen und Befehle im Hauptprogramm zu mischen, wurde abgesehen, damit der Nutzer seine Funktionen später in jedem Teil seines Programms gleichermaßen verwenden kann und sich nicht explizit Gedanken, über die die Reihenfolge seiner Prozedurdefinitionen sein muss. Abbildung \ref{fig:implementation:datastructure:grammar:1} zeigt den Einstieg in die Grammatik.
...
Der Knoten zur Prozedurdefinition (\inlinec{procedureDeclaration}) ist in Abbildung \ref{fig:implementation:datastructure:grammar:2} dargestellt. Dieser beinhaltet Terminalsymbole, welche für die Darstellung der Elemente im Drag \& Drop-Editor in BlattWerkzeug genutzt werden, für den Syntaxbaum jedoch nicht relevant sind. Mit den Terminalsymbolen wird das spätere Aussehen dem der Syntax von JavaScript nachempfunden. Dadurch soll der spätere Umstieg auf diese Universalsprache erleichtert werden. JavaScript im Speziellen wurde gewählt, da auch die Kompilierung in diese Sprache erfolgt (siehe \ref{sec:implementation:evaluation}).
...
\begin{figure}
  ...
  \label{fig:implementation:evaluation:while}
\end{figure}
