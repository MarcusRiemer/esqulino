...
Dieses Kapitel beschreibt die generellen Anforderungen an das Projekt. Dabei werden auch einige theoretische Hintergründe betrachtet, speziell didaktische Konzepte für die Vermittlung von SQL oder die Vorgehensweisen bei der Entwicklung von Oberflächen. Nicht Bestandteil dieses Kapitels ist eine ausführliche Analyse der Machbarkeit, den Schwerpunkt bildet also die Beschreibung eines didaktisch sinnvollen Konzeptes, welches dann möglicherweise nicht sofort mit allen Details umgesetzt werden kann. Auch die Planung konkreter Umsetzungsstrategien wird nicht vorgenommen, diese softwaretechnischen Aspekte werden in Kapitel \ref{sec:implementation-analysis}~\nameref{sec:implementation-analysis} besprochen. 
...
Ebenfalls aus Gründen der einfacheren Zugänglichkeit sollte eine einzelne Serverinstanz in der Lage sein, mehrere Projekte simultan zu bedienen. Die Lehrperson kann also mit einem einzigen Serverprozess eine ganze Klasse versorgen. Eine Begleiterscheinung dieser Anforderung ist allerdings die notwendige Implementierung von zumindest rudimentären Zugriffsbeschränkungen.
