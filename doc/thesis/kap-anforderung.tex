...
\item \texttt{SELECT} \\
  ...
  \end{enumerate}
\item \texttt{FROM} \\
  Auch die Auslassung dieser Komponente ist im Regelfall nicht zulässig. Die Funktionalität der komplexen \texttt{ON}-Bedingung \ref{feat:from-on-expression} hängt von den Möglichkeiten im  \texttt{WHERE} und \texttt{HAVING} ab.
  \begin{enumerate}
...
\begin{enumerate}
\item Funktionen \\
  Für Schüler ist zu erwarten, dass fast jede Funktion einer eigenen Einführung bedarf.
  \begin{enumerate}
  \item \label{feat:fun-length} \texttt{LENGTH}
  \end{enumerate}
\end{enumerate}
