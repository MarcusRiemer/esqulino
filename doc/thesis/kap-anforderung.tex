...
\subsection{Konzept für Oberflächen mit HTML}
\label{sec:design-ui-editor}

\warning{Diese Oberflächen werden nicht in reinem HTML verfasst, es kommen noch einige Erweiterungen in Form einer Templatingengine zum Einsatz. Diese technischen Details bleiben den Entwicklern allerdings verborgen, im Normalfall sehen sie nicht einmal den ``normalen'' HTML-Code. Ähnlich wie für die SQL-Abfragen wird also auch für die Oberflächen ein dezidierter Editor implementiert. Die Schüler sollten zu keinem Zeitpunkt selber Quelltexte tippen müssen, optional aber durchaus die Möglichkeit dazu haben.}
...
\begin{description}
  ...
  \item[Bilder] \hfill \\
    Technisch gesehen nicht nötig, aber für Schüler vermutlich motivierend, ist die Einbindung von Bildern.
\end{description}

Spätestens durch die Aufnahme von Bildern ist auch für statische Webseiten eindeutig, dass es möglich sein muss diese Bedienelemente über die Angabe von Parametern zu beinflussen. Praktisch werden diese Parameter im Regelfall jedoch erst zur Laufzeit durch die Ausführung einer Abfrage mit konkreten Werten angegeben.
...
Die grundsätzliche Struktur entspricht der eines Baumes mit einer Wurzel, also der Einstiegsseite. Unter diese Wurzelseite können dann weitere Seiten eingepflegt werden, die ggfs. auch Eigenschaften ihrer Eltern übernehmen können. Diese streng hierarchische Struktur macht es vergleichsweise leicht, optional ein sehr einfaches Templating-System zu definieren.

\subsection{Editor für Oberflächen}

\warning{Nicht alle hier beschriebenen Bedienelemente sind notwendigerweise in der Entwicklungsumbegung implementiert! Dieses Kapitel ist eine \textit{allgemeine} Betrachtung von didaktisch sinnvollen Bedienelementen einer Benutzeroberfläche. Um zu sehen, welche Funktionalität tatsächlich implemtiert worden ist lohnt sich ein Blick in Anhang \ref{app:implemented-features}:~\nameref{app:implemented-features}.}
