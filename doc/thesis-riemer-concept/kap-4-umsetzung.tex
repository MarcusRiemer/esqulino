...
Letzten Endes fiel die Entscheidung auf das Dateisystem. Die Gründe ähneln jenen, wie sie schon bei der Wahl von \texttt{SQLite} aufgezählt wurden (\ref{sec:implementation-database-system}~\nameref{sec:implementation-database-system}): Backups des Dateisystems sind trivial, der Betrieb außerordentlich eine, externe Programme sind zahlreich vertreten. Sollte eine komplizierte Lösung notwendig werden, kann man diese immer noch implementieren, sobald sie tatsächlich notwendig wird.
...
\item[Benutzerdatenbank] \hfill \\
  Die in Kapitel \ref{sec:concept-access-control}~\nameref{sec:concept-access-control} beschriebenen Zugriffskontrollen bedürfen einer rudimentären Benutzerdatenbank. Aktuell wird diese als Teil des Projekts gespeichert, die Passwörter werden dabei selbstverständlich verschlüsselt.
\end{description}
