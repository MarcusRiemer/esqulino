...
Die einzelnen Komponenten der Abfragen erben von einer gemeinsamen Basisklasse \texttt{Component}. Jede konkrete Komponente wird dann als eigene Klasse implementiert, Diagramm \ref{uml:class-diagram-sql-components} zeigt einen Ausschnitt der für \texttt{SELECT}-Abfragen nötigen Beteiligten. Die Methoden \texttt{toModel()} und \texttt{toSqlString()} dienen der Überführung des Syntaxbaumes in die Repräsentation für die \texttt{.json}- beziehungsweise die \texttt{.sql}-Dateien.
...
\begin{diagram}[p]
  \centering\begin{tikzpicture}
    ...
    \begin{abstractclass}[text width=9cm]{Query}{0, -3.5}
    ...
    \path[draw,-triangle 45] (ConstantExpression.west) -| ($(Query.south)+(-3,0)$);
    \path[draw] (ColumnExpression.west) -| ($(Query.south)+(-3,0)$);
    \path[draw] (ParameterExpression.west) -| ($(Query.south)+(-3,0)$);
    \path[draw] (StarExpression.west) -| ($(Query.south)+(-3,0)$);
    \path[draw] (BinaryExpression.west) -| ($(Query.south)+(-3,0)$);
 \end{tikzpicture}
...
Der wesentliche Inhalt des Rendervorgangs besteht im Sammeln der für die Anzeige notwendigen Daten, wie sie in Kapitel \fullref{sec:page-data-sources} beschrieben worden sind. Ein Großteil dieser Daten kann unmittelbar aus dem Datenmodell herausgelesen werden: Die Ruby-Klassen für Projekte und Seiten verfügen über dezidierte Methoden zur Bereitstellung der Daten ihres Namensraumes (Listing~\ref{lst:server-project-render-data}). Die \texttt{GET}-Daten werden unmittelbar von dem serverseitigen Framework zur Verfügung gestellt.
...
\begin{lstlisting}[float=h, language=Ruby, caption={Renderdaten einer Abfrage (2): Ausführen und transformieren}, label={lst:server-project-query-map-results}]
...
Der eigentliche Rendervorgang wird dann von der Liquid-Bibliothek durchgeführt. Deren Interface ist aber wie in Kapitel \fullref{sec:eval-templating-language} beschrieben denkbar einfach: Übergeben wird ein String mit dem zu rendernden Template und eine HashMap mit den zur Verfügung stehenden Daten.
