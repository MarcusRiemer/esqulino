...
Um das folgende Kapitel anschaulicher zu gestalten, werden alle Beispiele anhand eines sehr reduzierten Datenbestandes zu einem einfach Blog illustriert. Dieser Blog soll dem Endbenutzer über zwei Seiten näher gebracht werden. Eine Hauptseite dient der Auflistung aller Blogbeiträge welche im Einzelnen auf einer Detailseite in Gänze betrachtet und kommentiert werden können.
...
\item[\texttt{query} für \texttt{SELECT}-Abfragen] \hfill \\
  Nicht jede Webseite referenziert notwendigerweise alle verfügbaren Abfragen.
\item[\texttt{get} für \texttt{URL}-Parameter] \hfill \\
  Häufig sollen auf einer
