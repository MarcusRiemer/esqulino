...
\begin{document}
\subsection*{Generierung von syntaxfreien Editoren für beliebige Programmiersprachen}
...
\lstinputlisting[float=p,label={lst:partial-sql-grammar},caption={Auszug aus der Grammatik für \texttt{SQL}, die gezeigten Regeln definieren die \texttt{SELECT}-Komponente. Aus der vollständigen Grammatik (und zusätzlichen Stylingangaben) ist der Editor in Abbildung~\ref{fig:example-sql-ide} erzeugt worden.},captionpos=b,language={Grammar}]{sql.grammar}
