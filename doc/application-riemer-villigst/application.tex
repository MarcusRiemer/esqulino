...
Konventionelle Programmierwerkzeuge sind speziell auf die Bedürfnisse von professionellen Anwendern zugeschnitten. Aufgrund der damit verbundenen Komplexität eignen sich diese Programme daher aus didaktischer Sicht nur eingeschränkt für die Einführung in die Programmierung. Außerdem haben Studien gezeigt, dass die strengen syntaktischen Anforderungen von gängigen Programmiersprachen eine frustrierende Einstiegshürde darstellen. Trotzdem kommen die professionellen Werkzeuge, in der Regel aus Mangel an Alternativen, häufig im Unterricht zum Einsatz. Und darüber hinaus sind typische Einstiegsaufgaben häufig sehr künstlich und weit von der Lebenswirklichkeit der Lernenden entfernt, was sich ebenfalls negativ auf deren Motivation auswirkt\cite{resnick_scratch:_2009}.

Im Rahmen der Promotion soll daher erforscht und demonstriert werden, wie sich aus formalen Beschreibungen von Programmiersprachen benutzerfreundliche, syntaxfreie Editoren speziell für Lernende erzeugen lassen. Damit soll Lehrern ein Werkzeug an die Hand gegeben werden, die Einführung in die Programmierung (zum Beispiel mit \texttt{SQL}, \texttt{HTML}, \texttt{CSS} und \texttt{JavaScript}) anhand praktischer relevanter Projekte zu vermitteln.

Die erzeugten Editoren sollen im Regelfall existierende Sprachen augmentieren, deren Gestalt aber nicht verstecken. Obwohl die Anmutung eines generierten Editors also große Nähe zur textuellen Darstellung haben kann, erfolgt die Programmierung per Maus durch das Verschieben der dargestellten Blöcke. Abbildung~\ref{fig:example-sql-ide} zeigt ein prototypisches Beispiel für einen generierten \texttt{SQL}-Editor\footnote{Dieser und weitere Prototypen lassen sich unter \url{https://blattwerkzeug.de/villigst} ausprobieren.}. Die Konzeption dieser Editoren orientiert sich an den Erfahrungen, welche die Forscher hinter Projekten wie Scratch \cite{maloney_scratch:_2004} oder Googles Blockly \cite{fraser_ten_2015} gemacht haben.

Bei den als Eingabe fungierenden formalen Beschreibungen handelt es sich um eine Abwandlung typischer Grammatiken, wie sie zur Definition der Syntax in fast jedem Compiler zum Einsatz kommen \cite[S. 42ff]{aho_compilers:_2007}. Die automatisierte Generierung der Oberflächen aus den Grammatiken ermöglicht es erfahrenen Anwendern (zum Beispiel Lehrkräften) gegebene Programmiersprachen didaktisch motiviert einzuschränken. So wäre es zum Beispiel möglich, eine \texttt{SQL}-Variante zu erstellen, bei welcher die \texttt{GROUP BY}-Komponente nicht zur Verfügung steht. In Anlehnung an die bei \cite{klaeren_macht_2007} beschriebenen \enquote{Sprachebenen} soll sich so der Funktionsumfang der generierten Editoren an den Lernfortschritt der Klassengemeinschaft anpassen lassen.

\begin{figure}[h]
  \centering\includegraphics[width=\linewidth]{screenshot-drag-drop-ide.png}
  \caption{Generierter Editor für die Programmiersprache \texttt{SQL}, die gezeigte Abfrage sucht Charaktere aus der Buchserie \enquote{Die drei Fragezeichen}.}
