...
describe("Drop Target State", () => {
  describe("isChildRequired", () => {
    it('EmptyTree)', () => {
      ...
      expect(_isChildRequired(validator, inTree, []))
        .withContext("Empty trees always want insertion at the root")
        .toBe(true);
      expect(_isChildRequired(validator, inTree, [["lhs", 0]]))
        .withContext("Unknown parent, we don't assume something should be inserted")
        .toBe(false);
      expect(_isChildRequired(validator, inTree, [["lhs", 0], ["expr", 0]]))
        .withContext("Unknown parent, we don't assume something should be inserted")
        .toBe(false);
    });
    ...
    it('(TRUE AND <hole>)', () => {
      ...
      expect(_isChildRequired(validator, inTree, [["lhs", 0]]))
        ...
        .toBe(false);
      expect(_isChildRequired(validator, inTree, [["rhs", 0]]))
        .withContext("rhs is empty")
        .toBe(true);
      expect(_isChildRequired(validator, inTree, [["nonexistant", 0]]))
