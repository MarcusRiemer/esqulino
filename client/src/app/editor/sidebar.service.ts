...
export class SidebarService {
  ...
  showSingleSidebar(newType: string, param?: any): number {
    const ids = this.showMultiple([{ type: newType, param: param }], true);
  ...
  showAdditionalSidebar(newType: string, param?: any): number {
    const ids = this.showMultiple([{ type: newType, param: param }], false);
  ...
  showMultiple(mult: SidebarModel[], replace = true): number[] {
    console.log(
      `Requested new Sidebars: [${mult.map((s) => s.type).join(", ")}]`
    );
