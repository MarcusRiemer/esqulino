...
export class OverviewGrammarGraphQLComponent implements OnInit {
  ...
  ngOnInit() {
    this.availableGrammars = this._query
      .watch()
      .valueChanges.pipe(map((result) => result.data.grammars));
    this.availableGrammars
      .pipe(first())
      .subscribe(
        (grammars) =>
          (this.resultsLength = grammars.reduce((acc, _) => acc + 1, 0))
      );
  }
