...
export class ServerDataService {
  ...
  readonly getUserNewsDetails = new IndividualDescriptionCache<
    NewsFrontpageDescription
  ...
  updateNews(
    id: string,
    desc: NewsUpdateDescription
  ): Observable<NewsDescription> {
    // The given description may have to many fields, we need to strip
    // every unneeded field.
    desc = {
      publishedFrom: desc.publishedFrom || null,
  ...
  deleteNews(id: string): Observable<Object> {
    const toReturn = this._http.delete(this._serverApi.getNewsSingle(id)).pipe(
      tap((_) => {
