...
describe(`Request Cache`, () => {
  ...
  it(`Observable that does not complete`, () => {
    ...
    c.value.subscribe((v) =>
      fail(`No value should have been emitted, but got ${v}`)
    );
  });
  ...
  it(`Observable that does eventually complete`, () => {
    ...
    const subs = c.value.subscribe((v) =>
      fail(`No value should have been emitted, but got ${v}`)
    );
  ...
  it(`Observable that immediatly throws`, () => {
    ...
    c.inProgress
      .pipe(first())
      .subscribe((p) => expect(p).toBe(true, "Progress"));
  ...
  it(`Works for Angular HTTP requests`, async () => {
    ...
    httpTestingController.expectOne("/test").flush("response");

    inProgress = await c.inProgress.pipe(first()).toPromise();
    expect(inProgress).withContext("Answer received").toEqual(false);
  });
});
