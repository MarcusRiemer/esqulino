...
import { doesNodeMatchSelector, findMatches } from "./transform.matching";
import { arrayEqual } from "../util";
("./transform.description");

function haveSameParent(
  leftLoc: NodeLocation,
  rightLoc: NodeLocation
): boolean {
  let leftLocCopy = [...leftLoc];
  let rightLocCopy = [...rightLoc];

  leftLocCopy.pop();
  rightLocCopy.pop();

  let toReturn = true;
  for (let i = 0; i < leftLocCopy.length; i++) {
    toReturn = toReturn && leftLocCopy[i] === rightLocCopy[i];
  }
...
/**
 * Takes a parentNode and the location of a child node and a splitPropertyPattern defined by the user.
 * Splits the matched node into children nodes depending on the parameters of the splitPropertyPattern
 * and then appends the new children nodes to either the parentNode or under a wrapperNode,
 * that takes the place of the exploded node, if a wrapperNode is specified by the user.
 * @param parentNode  The parent of the node to be wrapped.
 * @param matchedNodeLoc  The location of the node matched where the transformation should happen.
 * @param splitPropertyPattern The pattern defined by the user for this transformation.
 * @returns The nodeDescription of the subtree after the transformation.
 */
...
export function splitOnPropertyTransformation(
  ...
  console.log(JSON.stringify(
    {
      location: matchedNodeLoc, 
      tree: subTree.toModel(),
    }
  ))
  */
  ...
  if (nodeToSplit.properties[splitPropertyPattern.propertyName]) {
    ...
    const delimiter =
      splitPropertyPattern.delimiter !== undefined
        ? splitPropertyPattern.delimiter
        : "";
    let splitPropResult =
      nodeToSplit.properties[splitPropertyPattern.propertyName].split(
        delimiter
      );
    if (splitPropertyPattern.deleteDelimiter === false) {
      for (let i = 0; i < splitPropResult.length - 1; i++) {
        // Reappend delimiter up to the last element, excluding the last element
        splitPropResult[i] =
          splitPropResult[i] + splitPropertyPattern.delimiter;
      }
    }
    ...
    for (let propValue of splitPropResult) {
      ...
      if (splitPropertyPattern.oldChildren === "copy") {
        temp = appendChildGroupsToNodeDescription(
          temp,
      ...
      if (splitPropertyPattern.otherProperties !== "ignore") {
        temp = appendPropertiesToNodeDescription(
          temp,
          nodeToSplit.properties,
          splitPropertyPattern.otherProperties
        );
      }
      newNodes.push(temp);
    }
    ...
    if (splitPropertyPattern.wraperNode) {
      ...
      wrapperNodeDesc = appendChildGroupsToNodeDescription(
        wrapperNodeDesc,
        newChildren,
        "end"
      );
...
/**
 * Takes the location of a matched syntax node and transform it or its
 * subtree according to the parameters defined in the transformation pattern.
 *
 * @param parentNode
...
export function applyTransformPatternOnLocation(
  ...
  */
  switch (tPattern.kind) {
    ...
    case "wrap":
      {
        let oldParentNode: SyntaxNode;
        let newParentNodeDesc: NodeDescription;
        // Check for root
        if (locationEquals(loc, [])) {
          oldParentNode = tree.rootNode;
          newParentNodeDesc = wrapTransformation(oldParentNode, [], tPattern);
        } else {
          oldParentNode = tree.locate(loc).nodeParent;
          newParentNodeDesc = wrapTransformation(
            oldParentNode,
            [loc[-1]],
            tPattern
          );
    ...
    case "merge":
      {
        ...
        */
        if (locationEquals(loc, [])) {
          // No merging to be done on the root level
          return tree.toModel();
        }
        let oldParentNode = tree.locate(loc).nodeParent;
...
export function applyRules(
  ...
  let rule_index = 0;
  let resultTreeDesc: NodeDescription = inp.toModel();
  let inputDesc = inp.toModel();
  let inpTree: SyntaxTree = inp;
  while (rule_index < rules.length) {
    //console.log("Input Tree: " + JSON.stringify(inpTree.toModel()))
    modifiedFlag = false;
    ...
    while (matches.length > 0) {
      ...
      if (
        ...
        // Specifically check for the merge transformation that the next match is an immediate sibling node
        if (
          rules[rule_index].transformPattern.kind === "merge" &&
          matches.length > 0
        ) {
          if ( //are immediateSiblings?
            haveSameParent(nextMatch, matches[0]) &&
            nextMatch[-1][1] === matches[0][-1][1] + 1
          ) {
            resultTreeDesc = applyTransformPatternOnLocation(
              inpTree,
    ...
    // If a modification was made, recheck the previous rules again.
    if (modifiedFlag) {
      rule_index = 0;
      inpTree = new SyntaxTree(inputDesc);
    } else rule_index++;
  }
  return new SyntaxTree(inputDesc);
}
