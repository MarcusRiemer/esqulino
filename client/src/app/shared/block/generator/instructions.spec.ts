...
describe("BlockLanguage GeneratorInstructions", () => {
  it("Hands out empty TypeInstructions as a default", () => {
    ...
    expect(
      inst.typeInstructions("missing", "irrelevant") instanceof TypeInstructions
    ).toBe(true);
    expect(
      inst.typeInstructions("g", "missing") instanceof TypeInstructions
    ).toBe(true);
  });
  ...
  describe("Instructions for single blocks", () => {
    ...
    it("Default values for missing scopes", () => {
      ...
      expect(bound.scopeBlock(0)).toEqual(
        DefaultInstructions.blockInstructions,
        "Block"
      );
      expect(bound.scopeIterator("missing")).toEqual(
        DefaultInstructions.iteratorInstructions,
        "Layout"
      );
      expect(bound.scopeTerminal("missing")).toEqual(
    ...
    it("Mixing specific and default values", () => {
      ...
      expect(bound.scopeTerminal("this")).toEqual(
        jasmine.objectContaining({
          style: {
            display: "block",
            color: "red",
