...
export class ResourceReferencesStaticService extends ResourceReferencesService {
  ...
  constructor(
    ...
    blockLanguages.forEach(
      (b) => (this._blockLanguages[b.id] = new BlockLanguage(b))
    );
  ...
  async ensureResources(req: RequiredResource | RequiredResource[]) {
    ...
    const ensurals = req.map((r) => {
      switch (r.type) {
        case "blockLanguage":
          return Promise.resolve(!!this._blockLanguages[r.id]);
        case "grammar":
          return Promise.resolve(!!this._grammars[r.id]);
