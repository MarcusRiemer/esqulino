module Resolvers
  ...
  class RelatedModelsVisitor < GraphQL::Language::Visitor
    ...
    def on_field(node, parent)
      ...
      if !parent.name.nil? && (@stack.last != parent)
    ...
    def stack_model_attributes(stack)
      ...
      stack.each do |stack_node|
        ...
        if model_attribute && !matching_association
        ...
        elsif in_model_associations && (stack_node.name != 'nodes')
          ...
          raise "Unknown attribute #{stack_node.name} in model association stack: #{stack_names}" unless stack_names.start_with? 'FullProject.project.assignmentSubmissions.assignmentSubmissionGradeParticipant'
    ...
    def add_nested_include(model_stack, curr)
      if model_stack.empty?
        raise raise 'Reached empty model stack for nested include'
      elsif model_stack.length == 1
        curr[model_stack.first.name.underscore] = {}
      else
