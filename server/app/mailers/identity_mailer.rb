class IdentityMailer < ApplicationMailer
  ...
  def reset_password(identity, locale)
    ...
    @password_reset_url = "http://#{locale}.#{@@base_url}/api/identities/reset_password/#{identity[:own_data]["password_reset_token"]}"
  ...
  def confirm_email(identity, locale)
    ...
    @user_verify_url = "http://#{locale}.#{@@base_url}/api/identities/confirmation/#{identity[:own_data]["verify_token"]}"
