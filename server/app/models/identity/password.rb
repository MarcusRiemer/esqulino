...
module Identity
  class Password < Identity
    ...
    def set_all_passwords(password)
      identities = Password.where('user_id = ?', self.user_id)
    ...
    def ensure_password_hashed!
      if (not password.nil?) and (not BCrypt::Password.valid_hash? password) then
        self.own_data["password"] = BCrypt::Password.create(password)
      end
    end
