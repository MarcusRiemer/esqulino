...
class Grammar < ApplicationRecord
  ...
  self.ignored_columns = %w[model extends_id]
  ...
  def all_types
    ...
    # must **not** be merged.
    foreign_types.merge(types) do |_key, foreign, local|
      foreign.merge(local)
    end
  end
  ...
  def all_visualisations
    ...
    # overwritten directly, but merged. But the types inside a language
    # must **not** be merged.
    foreign_visualisations.merge(visualisations) do |_key, foreign, local|
      foreign.merge(local)
    end
  end
  ...
  def regenerate_from_code_resource!(ide_service = IdeService.instance)
    compiled = generated_from.emit_ast!(ide_service)
    ...
    document_attributes_changed = !current_relevant_document.eql?(regenerated_attributes)
    ...
    if document_attributes_changed || document_references_changed
      assign_attributes regenerated_attributes if document_attributes_changed

      self.grammar_reference_origins = regenerated_relationships if document_references_changed
      ...
      block_languages.each do |b|
        affected << b if b.regenerate_from_grammar!
      end
    end
  ...
  def refresh_from_references!
    include_types = grammar_reference_origins.filter do |ref|
      ref.reference_type == 'include_types'
    end

    visualize_types = grammar_reference_origins.filter do |ref|
  ...
  def includes_references
    ...
    grammar_reference_origins.where(reference_type: include_types)
  end
  ...
  def visualizes_references
    ...
    grammar_reference_origins.where(reference_type: visualizes)
  end
