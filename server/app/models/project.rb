...
class Project < ApplicationRecord
  ...
  scope :full, -> {
    includes(:block_languages, :code_resources, :project_databases, :project_sources)
  }
  # A project with all associated resources that are used by **only** this
  # project and no other project.
  scope :with_exclusive, -> {
    includes(:code_resources, :project_databases, :project_sources, :project_uses_block_languages)
  }
  ...
  def database_or_default(database_id = nil)
    if database_id then
