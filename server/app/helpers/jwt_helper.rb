...
module JwtHelper
  ...
  def self.access_token_duration
    Rails.configuration.sqlino[:auth_tokens][:access_token].seconds
  end

  # Returns the default duration of an refresh token
  def self.refresh_token_duration
    Rails.configuration.sqlino[:auth_tokens][:refresh_token].seconds
  end
  ...
  def self.refresh_cookie_duration
    Rails.configuration.sqlino[:auth_tokens][:refresh_cookie].seconds
  end

  def self.append_registered_claims(payload = {}, duration = access_token_duration.from_now)
    payload.merge({ exp: duration.to_i, iss: JwtHelper.issuer })
  end
    ...
    unless @current_access_token
      ...
      if access_token || refresh_token
        begin
          ...
          # on login or logout)
          return nil unless attempt_refresh
  ...
  def clear_secure_cookies
    ...
    clear_current_refresh_token if refresh_cookie

    return unless access_cookie
