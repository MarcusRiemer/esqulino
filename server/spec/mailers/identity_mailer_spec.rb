...
RSpec.describe IdentityMailer, :type => :mailer do
  ...
  describe "Identity Mailer" do
    ...
    it "reset password" do
      ...
      expect(mail.text_part.body.to_s).to include("http://#{locale}.#{base_url}/api/identities/reset_password/#{identity[:own_data]["password_reset_token"]}")
    end
    ...
    it "confirm email" do
      ...
      expect(mail.text_part.body.to_s).to include("http://#{locale}.#{base_url}/api/identities/confirmation/#{identity[:own_data]["verify_token"]}")
    end
  end
end